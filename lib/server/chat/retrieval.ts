...
function builtInExperimentDefinition(question: string): string | null {
  ...
  if (/\belectric current\b|\bcurrent\b/.test(compact)) {
    return "In this DC shunt generator experiment, current is the flow of electric charge and is measured in amperes (A).";
  }
  if (/\bpotential difference\b|\bvoltage\b/.test(compact)) {
    return "In this experiment, voltage is the potential difference between two points, measured in volts (V).";
  }
...
export async function retrieveContext(question: string): Promise<RetrievedContext> {
  ...
  const builtIn = builtInExperimentDefinition(question);
  if (builtIn) {
